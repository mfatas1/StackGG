...
/**
 * Top players for a lane, ranked by winrate then games. Players with >= MIN games are
 * preferred; only if nobody clears MIN do we fall back to the small-sample field. Returns
 * up to `limit` (default 3): [leader, runner-up, third].
 */
function topForLane(rows: CrewRoleSpread[], lane: string, limit = 3): Best[] {
  const cands = rows
  ...
  return [...pool]
    .sort((a, b) => (b.cell.winrate ?? 0) - (a.cell.winrate ?? 0) || b.cell.games - a.cell.games)
    .slice(0, limit)
    .map((pick) => ({
...
/**
 * The crew's strongest picks in each lane, as a wide "epic" banner per role: the lane title
 * large, the lane's best player with their top-3 champs, and below it the 2nd/3rd-best
 * players (smaller) with their own best champions.
 */
export function LaneLeaders({ rows, crewSlug }: { rows: CrewRoleSpread[]; crewSlug: string }) {
  ...
  const best = LANES.map((l) => ({ lane: l, players: topForLane(rows, l.key) }));

  return (
    <div className="space-y-3">
      {best.map(({ lane, players }) => {
        ...
        return (
          <div key={lane.key} className="notch relative overflow-hidden border border-line/60 bg-surface-2/40">
            ...
            {/* left accent rail */}
            <div className={`absolute inset-y-0 left-0 w-1 ${leader && (leader.winrate ?? 0) >= 0.5 ? "bg-win/70" : "bg-loss/60"}`} />
            ...
            <div className="flex flex-col gap-4 p-4 pl-5 sm:flex-row sm:items-center sm:gap-5">
              ...
              {leader ? (
                <Link
                  href={playerHref({ riotId: leader.riotId, tag: leader.tag, region: leader.region, crewSlug })}
                  className="group flex items-center gap-3 sm:w-52 sm:shrink-0"
                >
                  <span className={`shrink-0 rounded-full p-[2px] transition-transform duration-150 group-hover:scale-105 ${(leader.winrate ?? 0) >= 0.5 ? "bg-win" : "bg-loss"}`}>
                    <ProfileIcon id={leader.profileIcon} name={leader.riotId} size={48} />
                  </span>
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold group-hover:text-gold">{leader.riotId}</div>
                    <div className="mt-0.5 font-mono text-2xs tnum">
                      <span className={`text-base font-bold ${wrTone(leader.winrate)}`}>{pct(leader.winrate)}</span>
                      <span className="text-ink-faint"> · {leader.games}g</span>
                    </div>
                  </div>
                </Link>
              ) : (
              ...
              {leader && leader.champions.length > 0 && (
                <div className="grid flex-1 grid-cols-3 gap-2">
                  {leader.champions.slice(0, 3).map((c) => {
            ...
            {/* Runner-ups: 2nd & 3rd best in the lane, smaller, with their own champs. */}
            {runners.length > 0 && (
              <div className="flex flex-col gap-2 border-t border-line/40 bg-bg/20 px-4 py-2 pl-5 sm:flex-row sm:flex-wrap sm:gap-x-8">
                {runners.map((p, i) => (
                  <RunnerRow key={`${p.riotId}#${p.tag}`} rank={i + 2} p={p} crewSlug={crewSlug} />
                ))}
              </div>
            )}
          </div>
...
/** A compact runner-up: rank badge, player, winrate, and their best champions as small chips. */
function RunnerRow({ rank, p, crewSlug }: { rank: number; p: Best; crewSlug: string }) {
  return (
    <div className="flex min-w-0 items-center gap-2.5 sm:flex-1">
      <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-bg/60 text-[10px] font-bold text-ink-faint tnum ring-1 ring-line/50">{rank}</span>
      <Link href={playerHref({ riotId: p.riotId, tag: p.tag, region: p.region, crewSlug })} className="group flex shrink-0 items-center gap-2 sm:w-40">
        <ProfileIcon id={p.profileIcon} name={p.riotId} size={28} />
        <div className="min-w-0">
          <div className="truncate text-xs font-medium group-hover:text-gold">{p.riotId}</div>
          <div className="font-mono text-[10px] tnum">
            <span className={wrTone(p.winrate)}>{pct(p.winrate)}</span>
            <span className="text-ink-faint"> · {p.games}g</span>
          </div>
        </div>
      </Link>
      <div className="flex min-w-0 flex-wrap items-center gap-1">
        {p.champions.slice(0, 3).map((c) => {
          const w = c.games ? c.wins / c.games : null;
          return (
            <span
              key={c.championName}
              className="flex items-center gap-1 rounded bg-bg/40 px-1 py-0.5 ring-1 ring-line/40"
              title={`${champName(c.championName)} · ${c.games}g · ${pct(w)}`}
            >
              <ChampIcon name={c.championName} size={18} />
              <span className="font-mono text-[10px] text-ink-faint tnum">{c.games}g</span>
            </span>
          );
        })}
      </div>
